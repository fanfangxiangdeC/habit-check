...
- 邮箱+密码登录，数据按账号隔离；支持添加 / 重命名 / 删除习惯，勾选与取消勾选，今日进度，连续天数，最近 7 天记录，多端实时同步

## 文件结构

```
index.html           页面结构（登录页 + 主界面）
css/style.css        样式（清新简洁风）
js/config.js         Supabase 连接配置（部署时填写）
js/app.js            页面逻辑（登录 + 勾选同步队列）
schema.sql           Supabase 建表脚本（新项目用，幂等，不含删数据）
migration_login.sql  v1→v2 一次性迁移（会清空旧数据）
```
...
> 老项目升级到 v2（已有旧数据）：在 SQL Editor 运行 `migration_login.sql`（会清空旧数据）。
...
### 3. 配置登录（Supabase 控制台）
1. **Authentication → Providers → Email**：确认 **Enable Sign up** 已开启。
2. **Authentication → URL Configuration**：把 **Site URL** 设为你的网页地址（如 `https://<用户名>.github.io/habit-check/`），并在 **Redirect URLs** 里加入同一个地址（忘记密码/确认邮件的跳转需要它）。
3. 邮箱确认：默认开启（注册后需点邮件确认链接）；如想注册后立即登录，可在 Email 设置里关闭 **Confirm email**。
...
- 打开网页先登录/注册（邮箱+密码）；注册后注意查收确认邮件（如已开启邮箱确认）。
- 点右上角 **+** 添加习惯（填名称，可选 emoji 图标）。
- 点习惯名称可直接重命名；点卡片右侧 **⋯** 菜单可删除。
- 每天完成一项就点一下圆形勾选按钮；再次点击取消，快速连续点击也不会出错。
- 页面跨零点会自动切换到新的一天，多端登录同一账号时改动会实时同步。
